...
evaluate_simulations <- function(tfine, sim_al_data, sim_data, acd_fda_2, plot = FALSE, plot_subjects = NULL) {
  ...
  for (i in seq_len(n_sub)) {
    ...
    # Function to compute perpendicular distance from a point to the line
    ...
    mse_vec[i] <- mean((rowMeans(sim_vals[, top_idx, drop = FALSE]) - obs_vals)^2)
  }
  
  # --- Step 4: ISE computation ---
  ...
  ise_from_mean <- calc_ise(true_phi = sim_data$Y_no_noise, est_phi = mean_fd_mat)
  ise_bet_means <- calc_ise(true_phi = rowMeans(sim_data$Y_no_noise), est_phi = mean_fd)
  ...
  if (plot) {
    ...
    ise_df <- data.frame(subject = seq_along(ise_vec), ise = ise_vec)
    bp_stats <- boxplot.stats(ise_vec)
    outliers <- bp_stats$out
    
    ise_df <- ise_df %>%
      mutate(is_outlier = ise %in% outliers)
    
    q <- ggplot(ise_df, aes(x = "", y = ise)) +
      geom_boxplot(outlier.shape = NA, fill = "lightgray") +
      geom_jitter(aes(color = is_outlier), width = 0.03, size = 2, alpha = 0.6) +
      geom_text(data = filter(ise_df, is_outlier),
                aes(label = subject), 
                hjust = -0.3, color = "red", size = 3.5) +
      scale_color_manual(values = c("black", "red"), guide = "none") +
      labs(title = "ISE Distribution Across Subjects", y = "ISE", x = "") +
      theme_minimal()
    print(q)
    
    # --- SE boxplot ISE Mean function vs true trajectories---
    Q1 <- quantile(ise_from_mean, 0.25)
    Q3 <- quantile(ise_from_mean, 0.75)
    IQR_val <- Q3 - Q1
    upper_whisker <- Q3 + 1.5 * IQR_val
    
    mse_df <- data.frame(
      subject = seq_along(ise_from_mean),
      mse = ise_from_mean,
      is_outlier = ise_from_mean > upper_whisker
    )
    
    q_ise_mean <- ggplot(mse_df, aes(x = "", y = mse)) +
      geom_boxplot(outlier.shape = NA, fill = "lightgray") +
      geom_jitter(aes(color = is_outlier), width = 0.03, size = 2, alpha = 0.6) +
      geom_text(data = filter(mse_df, is_outlier),
                aes(label = subject), 
                hjust = -0.3, color = "red", size = 3.5) +
      scale_color_manual(values = c("black", "red"), guide = "none") +
      labs(title = "ISE: Mean Function vs True Trajectories", y = "ISE", x = "") +
      theme_minimal()
    print(q_ise_mean)
    
    # --- MSE distribution ---
    mse_df <- data.frame(subject = seq_along(mse_vec), mse = mse_vec)
    bp_stats_mse <- boxplot.stats(mse_vec)
    outliers_mse <- bp_stats_mse$out
    
    mse_df <- mse_df %>%
      mutate(is_outlier = mse %in% outliers_mse)
    
    q_mse <- ggplot(mse_df, aes(x = "", y = mse)) +
      geom_boxplot(outlier.shape = NA, fill = "lightgray") +
      geom_jitter(aes(color = is_outlier), width = 0.03, size = 2, alpha = 0.6) +
      geom_text(data = filter(mse_df, is_outlier),
                aes(label = subject), 
                hjust = -0.3, color = "red", size = 3.5) +
      scale_color_manual(values = c("black", "red"), guide = "none") +
      labs(title = "Mean of Squared Errors (MSE) per Subject", y = "MSE", x = "") +
      theme_minimal()
    print(q_mse)
  }
  ...
  return(list(
    ...
    ise_per_subject = ise_vec,
    ise_from_mean = ise_from_mean,
    ise_bet_means = ise_bet_means,
    mse_vec = mse_vec
  ))
}
